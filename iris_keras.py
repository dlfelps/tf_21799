...
def main(unused_argv):
  ...
  input_0 = tf.keras.Input(shape=(1,), name=FEATURE_KEYS[0])
  input_1 = tf.keras.Input(shape=(1,), name=FEATURE_KEYS[1])
  input_2 = tf.keras.Input(shape=(1,), name=FEATURE_KEYS[2])
  input_3 = tf.keras.Input(shape=(1,), name=FEATURE_KEYS[3])
  input = tf.keras.layers.concatenate([input_0, input_1, input_2, input_3])
  ...
  train_input_fn = input_fn(IRIS_TRAINING, num_training_data, batch_size=32,
                            is_training=True)
  classifier.fit(train_input_fn(), steps_per_epoch=4000, epochs=1)

  # Eval.
  test_input_fn = input_fn(IRIS_TEST, num_test_data, batch_size=30,
                           is_training=False)
  x_pred = classifier.predict_on_batch(test_input_fn())
